      ...
      geocoder.geocode(
        ...
        (results, status) => {
          if (status === "OK" && results[0]) {
            ...
            let destinationName =
              (isSubmitted && address) || "your selected destination";
      ...
      (results, status) => {
        if (status === "OK" && results[0]) {
          ...
          document.getElementById(
            "geo_locate"
          ).innerHTML = `Latitude: ${lat} Longitude: ${lng} <br> Your Location: ${address}`;
...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    if (firstDropdown == "green_house") {
      ...
      if (
        iconStack.at(-1) == "green_house" ||
        iconStack.at(-1) == "gree_arrow"
      ) {
        removeMarker("Search Result");
